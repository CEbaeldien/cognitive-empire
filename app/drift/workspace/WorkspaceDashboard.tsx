...
import ImportCSV from "@/app/admin/drift/ImportCSV";
...
const Sparkline = () => (
  <svg width="96" height="36" viewBox="0 0 96 36" fill="none" style={{ opacity: 0.7 }}>
    <defs>
      <linearGradient id="wsSpkGrad" x1="0" y1="0" x2="0" y2="1">
    ...
    <path d="M0 26 L12 18 L20 22 L32 10 L44 16 L56 8 L68 13 L80 4 L92 10 L96 7 L96 36 L0 36 Z" fill="url(#wsSpkGrad)" />
  </svg>
);

// ── Main component ─────────────────────────────────────────────────────────────

export default function WorkspaceDashboard({ data }: { data: DriftOverview }) {
  ...
  const sortedInterventions = [...data.interventions].sort((a, b) =>
  ...
  const revenueAtRisk = data.opportunities.reduce((s, o) => s + Number(o.score?.revenue_at_risk ?? 0), 0);
  const exposurePct = portfolioValue > 0 ? Math.min(100, (revenueAtRisk / portfolioValue) * 100) : 0;
  const criticalCount = data.opportunities.filter(o => o.score?.drift_level === "critical").length;
  ...
  const scored = data.opportunities.filter(o => o.score !== null).length;
  const opConf = data.opportunities.length > 0 ? Math.round((scored / data.opportunities.length) * 100) : 100;

  if (!authReady) {
    return (
      <div className="flex h-screen items-center justify-center bg-[#0b0f1c]">
    ...
    <div className="flex h-screen overflow-hidden bg-[#0b0f1c] text-white antialiased">
      ...
      <aside className="flex h-full w-56 shrink-0 flex-col bg-[#090c17] border-r border-white/[0.06]">
        ...
        <div className="px-5 pb-2 pt-1">
          <p className="text-[9px] font-medium uppercase tracking-[0.5em] text-slate-600">Workspace</p>
          <p className="mt-0.5 text-[11px] font-medium text-slate-400 truncate">{data.workspace.name}</p>
        </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div className="min-w-0">
                <p className="text-[12px] font-medium text-slate-200 truncate">{userName}</p>
          ...
          <main className="flex-1 overflow-y-auto px-8 py-6">
            ...
            {sortedInterventions.length === 0 ? (
              <div className="rounded-xl border border-white/[0.06] py-16 text-center">
                ...
                <p className="mt-1.5 text-[11px] text-slate-700">Import a snapshot to generate intervention actions.</p>
              </div>
            ) : (
              ...
              {leaking.length === 0 ? (
                ...
                <>
                  <div className="mb-2 grid gap-x-2" style={{ gridTemplateColumns: "1.2fr 0.9fr auto 1.1fr" }}>
                    {["Opportunity", "Account", "Exposure", "Why It Leaked"].map((h, i) => (
