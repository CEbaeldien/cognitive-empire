...
export default function DriftDashboard({ data }: { data: DriftOverview }) {
  ...
  // ── Filtered subsets ────────────────────────────────────────────────────────
  const filteredOpps = selectedClient
    ? data.opportunities.filter((o) => o.accounts?.name === selectedClient)
    : data.opportunities;

  const filteredInterventions = selectedClient
    ? data.interventions.filter(
        (iv) => iv.opportunities?.accounts?.name === selectedClient
      )
    : data.interventions;

  // ── KPIs derived from filtered data ─────────────────────────────────────────
  const portfolioValue = filteredOpps.reduce((s, o) => s + Number(o.value ?? 0), 0);
  const revenueAtRisk = filteredOpps.reduce(
    (s, o) => s + Number(o.score?.revenue_at_risk ?? 0),
    0
  );
  const exposurePct =
    portfolioValue > 0 ? Math.min(100, (revenueAtRisk / portfolioValue) * 100) : 0;
  const criticalDrift = filteredOpps.filter(
    (o) => o.score?.drift_level === "critical"
  ).length;
  const pendingCount = filteredInterventions.length;

  // ── Snapshot Freshness (3-tier, workspace-wide) ───────────────────────────
  const latestScoredAt =
    data.opportunities
      .map((o) => o.score?.scored_at)
      .filter(Boolean)
      .sort()
      .at(-1) ?? null;
  const snapshotAge = timeAgo(latestScoredAt);
  const ageMs = latestScoredAt
    ? Date.now() - new Date(latestScoredAt).getTime()
    : null;
  const freshnessColor =
    ageMs === null
      ...
      : "text-red-400";
  const freshnessTier =
    ageMs === null ? "none" : ageMs < 3_600_000 ? "green" : ageMs < 86_400_000 ? "amber" : "red";

  // ── Revenue Leaking — decay detected in last 7 days ──────────────────────
  const sevenDaysAgo = Date.now() - 7 * 86_400_000;
  const leaking = filteredOpps
    .filter((o) => {
      if (!["watch", "decaying", "critical"].includes(o.score?.drift_level ?? ""))
        return false;
      if (!o.score?.scored_at) return false;
      return new Date(o.score.scored_at).getTime() >= sevenDaysAgo;
    })
    .sort(
      (a, b) =>
        Number(b.score?.revenue_at_risk ?? 0) - Number(a.score?.revenue_at_risk ?? 0)
    )
    .slice(0, 5);

  // ── Lookup maps (full data, for score join) ───────────────────────────────
  const scoreByOppId = new Map(data.opportunities.map((o) => [o.id, o.score]));
  const oppById = new Map(data.opportunities.map((o) => [o.id, o]));

  // ── Intervention queue sorted by drift score desc ────────────────────────
  const sortedInterventions = [...filteredInterventions].sort((a, b) => {
    const sa = Number(scoreByOppId.get(a.opportunity_id)?.drift_score ?? 0);
    const sb = Number(scoreByOppId.get(b.opportunity_id)?.drift_score ?? 0);
    return sb - sa;
  });

  // ── Sidebar clients (grouped by account) ─────────────────────────────────
  // Display: sum of opportunity value; sort: by drift level then risk
  const accountMap = new Map<
    string,
    { name: string; level: string; risk: number; valueSum: number }
  >();
  for (const opp of data.opportunities) {
    ...
    if (!existing) {
      accountMap.set(name, { name, level, risk, valueSum: val });
    } else {
      const maxLevel =
        (LEVEL_ORDER[level] ?? 0) > (LEVEL_ORDER[existing.level] ?? 0)
          ? level
          : existing.level;
      accountMap.set(name, {
        name,
        ...
        valueSum: existing.valueSum + val,
      });
    }
  }
  const sidebarClients = Array.from(accountMap.values()).sort(
    (a, b) =>
  ...
  const scoredCount = filteredOpps.filter((o) => o.score !== null).length;
  const opConf =
    filteredOpps.length > 0
      ? Math.round((scoredCount / filteredOpps.length) * 100)
      : 100;
  const confColor =
    opConf >= 90 ? "text-emerald-400" : opConf >= 60 ? "text-amber-400" : "text-red-400";

  return (
    <div className="flex h-screen overflow-hidden bg-[#080d1a] font-sans text-slate-100">

      {/* ──────────────────────── SIDEBAR ──────────────────────── */}
      <aside className="flex h-full w-56 shrink-0 flex-col border-r border-[#1e2a45] bg-[#090d1e]">
        ...
        <div className="mx-4 border-t border-[#1e2a45]" />
        ...
        <div className="px-4 py-3">
          ...
          <button
            ...
            className={`flex w-full items-center justify-between rounded-md px-2 py-1.5 transition-colors hover:bg-slate-800/40 ${selectedClient === null ? "bg-[#1e3a5f]" : ""}`}
          >
        ...
        <nav className="px-3 py-3">
          ...
          <div className="mb-0.5 flex items-center gap-2.5 rounded-md bg-[#1e3a5f] px-3 py-2">
          ...
          {sidebarClients.length === 0 ? (
            ...
            sidebarClients.map((client) => (
              <button
                ...
                className={`flex w-full items-center justify-between rounded-md px-2 py-1.5 text-left transition-colors hover:bg-slate-800/40 ${
                  selectedClient === client.name ? "bg-[#1e3a5f]" : ""
                }`}
              >
                ...
                {client.valueSum > 0 && (
                  <span
                    className={`ml-1.5 shrink-0 text-[10px] ${EXPOSURE_TEXT[client.level] ?? "text-slate-600"}`}
                  >
                    {money(client.valueSum)}
                  </span>
                )}
              </button>
            ))
        ...
        {/* Profile */}
        <div className="shrink-0 border-t border-[#1e2a45] px-4 py-3.5">
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        {/* ── TOP HEADER ── */}
        <header className="flex shrink-0 items-center justify-between border-b border-[#1e2a45] bg-[#080d1a] px-7 py-4">
          <div>
            <h1 className="text-[20px] font-semibold tracking-tight text-white">
              Revenue Execution
              {selectedClient && (
                <span className="ml-3 text-[14px] font-normal text-slate-500">
                  — {selectedClient}
                </span>
              )}
            </h1>
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="relative">
              <div className="flex h-8 w-8 items-center justify-center rounded border border-[#1e2a45] bg-[#0f1629] text-slate-500">
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <main className="flex-1 overflow-y-auto px-7 py-6">
            ...
            <div className="mb-6 grid grid-cols-4 gap-4">
              ...
              {/* Revenue Exposure */}
              <div className="rounded-lg border border-[#1e2a45] bg-[#0f1629] px-5 py-4">
              ...
              <div className="rounded-lg border border-[#1e2a45] bg-[#0f1629] px-5 py-4">
                ...
                <p className={`mt-2.5 text-[24px] font-semibold leading-none ${criticalDrift > 0 ? "text-red-400" : "text-slate-600"}`}>
                  {criticalDrift}
                </p>
                <p className="mt-1.5 text-[11px] text-slate-600">
                  {criticalDrift === 1 ? "opportunity" : "opportunities"}
                </p>
              </div>
              ...
              <div className="rounded-lg border border-[#1e2a45] bg-[#0f1629] px-5 py-4">
                ...
                <p className={`mt-2.5 text-[24px] font-semibold leading-none ${pendingCount > 0 ? "text-orange-400" : "text-slate-600"}`}>
                  {pendingCount}
                </p>
              ...
              <div className="rounded-lg border border-[#1e2a45] bg-[#0f1629] px-5 py-4">
                <div className="flex items-start justify-between">
                  ...
                  {freshnessTier === "green" ? (
                    ...
                    <svg className="mt-0.5 h-4 w-4 shrink-0 text-amber-500/70" viewBox="0 0 20 20" fill="currentColor">
                      <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clipRule="evenodd" />
                    </svg>
                  ) : (
                    <svg className="mt-0.5 h-4 w-4 shrink-0 text-red-500/70" viewBox="0 0 20 20" fill="currentColor">
                      <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
                    </svg>
                  )}
                </div>
                <p className={`mt-2.5 text-[24px] font-semibold leading-none ${freshnessColor}`}>
                  {snapshotAge}
                </p>
            ...
            <section>
              ...
              {sortedInterventions.length === 0 ? (
                <div className="rounded-lg border border-[#1e2a45] px-8 py-14 text-center">
                  <p className="text-sm text-slate-600">No pending interventions.</p>
                  <p className="mt-1 text-[11px] text-slate-700">
                    {selectedClient
                      ? `No interventions for ${selectedClient}.`
                ...
                <div className="overflow-hidden rounded-lg border border-[#1e2a45]">
                  {/* Table header */}
                  <div className="grid grid-cols-[1.8fr_0.85fr_0.85fr_1.7fr_1.8fr_0.8fr_auto] border-b border-[#1e2a45] bg-[#0f1629]">
                  ...
                  {sortedInterventions.map((iv, idx) => {
                    ...
                    return (
                      <div
                        key={iv.id}
                        className={`grid grid-cols-[1.8fr_0.85fr_0.85fr_1.7fr_1.8fr_0.8fr_auto] items-start border-l-2 ${LEVEL_BORDER[level] ?? "border-l-slate-800"} ${idx > 0 ? "border-t border-[#1e2a45]/60" : ""} bg-[#090d1e] transition-colors hover:bg-[#0e1428]`}
                      >
                        {/* Opportunity */}
                  ...
                  {/* Table footer */}
                  <div className="flex items-center justify-between border-t border-[#1e2a45] bg-[#0f1629] px-5 py-2.5">
                    <p className="text-[11px] text-slate-600">
                      Showing {sortedInterventions.length} of {data.interventions.length} interventions
                      {selectedClient && ` · filtered to ${selectedClient}`}
                    </p>
          ...
          {/* ── RIGHT COLUMN ── */}
          <div className="w-[272px] shrink-0 overflow-y-auto border-l border-[#1e2a45] px-5 py-6">
                ...
                <div className="space-y-3">
                  {sortedInterventions.slice(0, 3).map((iv) => {
                    ...
                    return (
                      <div
                        key={iv.id}
                        className="rounded-lg border border-[#1e2a45] bg-[#0f1629] p-3.5"
                      >
            ...
            <div className="mb-5 border-t border-[#1e2a45]" />
                  ...
                  <div>
                    {leaking.map((opp, idx) => {
                      ...
                      return (
                        <div
                          key={opp.id}
                          className={`grid grid-cols-[1fr_auto_1.2fr] gap-x-3 py-2.5 ${idx > 0 ? "border-t border-[#1e2a45]/60" : ""}`}
                        >
        ...
        {/* ── STATUS BAR ── */}
        <footer className="flex shrink-0 items-center justify-between border-t border-[#1e2a45] bg-[#080d1a] px-7 py-2">
